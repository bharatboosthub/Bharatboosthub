...
/**
 * This is the "Auth Ready Gate". It returns a promise that resolves
 * only when Firebase has finished its initial authentication check.
 * This is the most important function for fixing the redirect issue.
 */
const onAuthReady = () => {
  return new Promise((resolve) => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      unsubscribe(); // We only need this to run once on page load
...
export {
    ...
    db,
    storage,
    onAuthReady, // Export the new gate function
    getCurrentUserData,
    GoogleAuthProvider,
    signInWithPopup,
    signOut,
...
This file will now wait for the "gate" from `firebase.js` to open before it does anything else. This ensures it always knows the correct login status.
...
After you replace both `js/firebase.js` and `js/main.js` with these new versions, the redirect issue will finally be solved. This "auth gate" pattern is the standard and most reliable way to handle authentication in modern web ap
